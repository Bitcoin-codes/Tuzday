// Flutter-native code editor screen with Run/Save/Open and Python runner integration
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
class _EditorScreenState extends State<EditorScreen> {
  ...
  void initState() {
    ...
    // Initialize the python runner (hidden WebView)
    PythonRunner().init();
  ...
  Future<void> _runCode() async {
    ...
    try {
      final result = await PythonRunner().runCode(code);
      setState(() {
        _output = '--- stdout ---\n${result['stdout'] ?? ''}\n--- stderr ---\n${result['stderr'] ?? ''}';
      });
    } catch (e) {
      setState(() {
        _output = 'Run failed: $e';
      });
    } finally {
      setState(() {
        _isRunning = false;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Hidden Pyodide runner (kept mounted)
          Offstage(
            offstage: true,
            child: SizedBox(
              width: 0,
              height: 0,
              child: PythonRunnerWidget(),
            ),
          ),
        ],
      ),
